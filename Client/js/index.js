...
socket.on('change-player', (names, firstMove = false) => {
	...
	if (names[start.activePlayer] === name) newMessage('Bot', 'Twój ruch', start.playersArray[start.activePlayer].playerSkin);
	else newMessage('Bot', `Ruch gracza ${names[start.activePlayer]}`, start.playersArray[start.activePlayer].playerSkin);
	put.isMoved = false;
