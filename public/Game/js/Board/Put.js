...
class Put {
  ...
  slide = (putData) => {
    ...
    const blockArrow = document.querySelector(
      `.arrows [data-${secondDirection}="${toBlock}"][data-${mainDirection}="${info[mainDirection]}"]`
    );
    ...
    this.size = this.movingField.offsetWidth + 1;
    let allLine = document.querySelectorAll(
      `.board div[data-${mainDirection}="${info[mainDirection]}"]`
    );
    allLine.forEach(
      (el) => (el.style[topOrLeft] = `${transformValue * this.size}px`)
    );
  ...
  putMovingElement = (putData, line, direction, transformValue) => {
    const elementArr = [...document.querySelectorAll(`.board-arrows`)].filter(
      (el) => {
        return (
          el.dataset.row == putData.row && el.dataset.column == putData.column
        );
      }
    );
  ...
  comeBack = (element, line, last, transformValue) => {
    ...
    line.forEach(
      (el) => (el.style.transition = "left .4s linear,top .4s linear")
    );
  ...
  replaceAllLine(line) {
    ...
    for (let i = 0; i < line.length; i++)
      info.push(this.getElementProperties(line[i]));
